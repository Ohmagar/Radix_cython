...
@cython.boundscheck(False)
@cython.wraparound(False)
cdef int pre_sort_digits(int *unsorted_first_digits, size_t data_start, size_t data_stop, size_t number_of_digits, Py_ssize_t length, int idx):# nogil:
    cdef size_t flooring = 10 ** number_of_digits
    cdef int *sorted_first_digits = <int *> malloc(length * 10 * sizeof(int))
    cdef size_t *bucket_indexes = <size_t *> malloc(10 * sizeof(size_t))
    cdef size_t i
    cdef size_t first_digit
    cdef int current_element
    cdef size_t data_idx = 0
    
    cdef size_t start
    cdef size_t stop
    cdef size_t count_of_digits
